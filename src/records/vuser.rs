...
impl Data {
    pub async fn passwd_verify(db: &PgPool, interface: &AddInterface) -> Result<i32, Error> {
        use Error::{BcryptError, DatabaseError, NoUserError, PasswordIncorrectError};
        ...
        match user {
            ...
            Some(user) => {
                let hashed = verify(interface.passwd.clone(), &user.passwd).map_err(BcryptError)?;

                if hashed {
                    Ok(user.id)
                } else {
                    Err(PasswordIncorrectError)
                }
            }
        }
    }
    ...
    pub async fn add(db: &PgPool, interface: &AddInterface) -> Result<PgQueryResult, Error> {
        ...
        let passwd = hash(interface.passwd.clone(), DEFAULT_COST).map_err(BcryptError)?;
