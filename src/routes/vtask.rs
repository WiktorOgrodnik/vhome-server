...
pub async fn all(request: crate::Request) -> tide::Result {
    ...
    let list_result: VResult = request
        .param("list_id")
        .ok()
        .map_or(VResult::None, |t| {
            VResult::Ok(t.parse::<i32>().expect("list id has to be a number!"))
        })
        .authorize(&request, AuthorizeLevel::Show)
        .await;

    Ok(match list_result {
        VResult::Forbidden | VResult::NotFound => tide::Response::new(404),
        list_id => tide::Response::builder(200)
            .body(tide::Body::from_json(
                &vtask::Data::all(&request.state().db, group_id, list_id.ok()).await?,
            )?)
...
pub async fn set_completed(request: crate::Request) -> tide::Result {
    ...
    let _ =
        vtask::Data::set_completed_guarded(&request.state().db, task_id, value, group_id).await?;
...
pub async fn add(mut request: crate::Request) -> tide::Result {
    ...
    let authorization: bool =
        vlist::Data::has_permission(&request.state().db, group_id, task_set_id).await;

    if !authorization {
        return Ok(tide::Response::new(StatusCode::Forbidden));
    }

    let _ = vtask::Data::add(&request.state().db, task_set_id, payload).await?;
...
pub async fn delete(request: crate::Request) -> tide::Result {
    ...
    let _ = vtask::Data::delete(&request.state().db, task_id).await?;
